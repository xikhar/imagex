...
- Frames are visual grouping containers, not React Flow parent nodes. Frame auto-sizing must reserve the header band so members cannot overlap it, and live frame dragging should move the frame plus unselected members as one transient `FlowStore` node update.
