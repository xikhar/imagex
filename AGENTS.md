...
- Dragging a frame member should keep it in that frame and resize the frame around it during the drag. Only explicit detach removes `frameId`; a detached node can stay outside, or reattach on drop when its center or most of its body is inside a frame.
